...
describe("AzureDevOpsWdioService", () => {
  ...
  beforeEach(() => {
    ...
    mockTestApi.updateTestResults.mockImplementation(async (results) => results);
  });
  ...
  test("afterTest results are published into the run created by onPrepare", async () => {
    ...
    await service.afterTest({ title: "C1234 login" }, {}, {
      passed: true,
      duration: 120,
    });
    await service.afterTest({ title: "C5678 checkout" }, {}, {
      passed: false,
      duration: 340,
    ...
    expect(mockTestApi.updateTestResults).toHaveBeenCalledWith(
      expect.arrayContaining([
        expect.objectContaining({ id: 1, outcome: "Passed", durationInMs: 120 }),
        expect.objectContaining({
          id: 2,
          outcome: "Failed",
          errorMessage: "boom",
  ...
  test("publishing from a worker reuses the run id from the environment", async () => {
    ...
    await worker.afterTest({ title: "C1234 login" }, {}, {
      passed: true,
      duration: 10,
    });
    await worker.after();
  ...
  test("ignores tests without a case id in the title", async () => {
    ...
    await service.afterTest({ title: "untagged test" }, {}, {
      passed: true,
      duration: 10,
    });
    await service.after();
